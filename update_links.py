...
def get_atcoder_contest_details(task_url: str) -> Contest:
  ...
  if response.status_code != 200:
    raise RuntimeError(f"Failed to get {task_url}. Status code {response.status_code}.")
...
def get_codeforces_contest_details(contest_url: str) -> Contest:
  ...
  if response.status_code != 200:
    raise RuntimeError(f"Failed to get {contest_url}. Status code {response.status_code}.")
  ...
  contest = Contest(
    contest_code=f"CFC{contest_code}",
    contest_name=contest_name
  )
...
def cache_codeforces_contests(contest_codes: list[str], prefix: str):
  task_urls = [f"https://codeforces.com/contest/{contest_code}" for contest_code in contest_codes]
  contest_df = pd.read_csv(CONTESTS_CACHE)
  cached_contests = set(contest_df["contest_code"].values)

  is_cached_list = [f"{prefix}{contest_code}" in cached_contests for contest_code in contest_codes]

  contests = []
  for url, is_cached in tqdm.tqdm(list(zip(task_urls, is_cached_list)), desc="Caching Codeforeces"):
    if is_cached:
      print(f"Using cache for {url}.")
      continue
